...
4. **`agent.py::extract_receipt()`** — the public API. Under `LLM_PROVIDER=mock` (or `provider="mock"`) it short-circuits to `_mock_extraction()`; otherwise it resolves provider+model, base64-encodes the image, builds the provider-specific content block via `_build_image_content()`, and calls Instructor's unified `client.create(response_model=ExtractedReceipt, ...)` — the same call shape regardless of provider.
5. **`agent.py::_build_image_content()`** — the one place provider differences actually show up: OpenAI's `image_url` data-URI format vs. Anthropic's `image`+`source` format (Gemini currently reuses OpenAI's shape via Instructor's provider-agnostic message handling — flagged as unverified against a real API call, see the module docstring).
...
9. **`agent.py::main()`** — CLI entry point via `python -m agent`. `--provider`/`--model` flags override the env-var resolution per-invocation. `nargs="?"` on the `image` arg lets `--ui` launch standalone (fixed in F1.2 review S1).
