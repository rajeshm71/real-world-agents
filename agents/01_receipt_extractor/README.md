...
**PDF support is implemented but not yet verified against a live API call** — no OpenAI or Anthropic API key was available in the sandbox this was built in. It uses Instructor's own cross-provider `PDF` multimodal helper (`instructor.processing.multimodal.PDF`), confirmed to exist with the expected `from_base64()` signature against the actually-installed library, but the real end-to-end request/response has not been exercised. Treat it the same as the other real-key-only verification items below until someone runs it once.
