...
**Provider note:** #04 is OpenAI-only in v1. The `openai-agents` SDK is provider-agnostic via [LiteLLM](https://docs.litellm.ai/), so swapping in Claude or Gemini is a one-line change — but LiteLLM is deferred from v1 to keep the dep count tight. See the SDK's provider docs when you want to swap.
...
7. **`agent.py::_looks_like_meeting_notes()`** — R5 case 1 gate. Below `MIN_NOTES_CHARS=200` OR no commitment verbs (`will`, `agreed to`, `action item`, etc.) detected → refuse with friendly message before Agent construction, so bogus input costs nothing.
...
- **Extremely short meetings** — the R5 case 1 gate (`< 200 chars` OR no commitment verbs) rejects fragmentary input. A real one-sentence follow-up ("Sync with Legal by Friday.") gets refused because it's under threshold. Workaround: pad with context, or lower `MIN_NOTES_CHARS` in `agent.py` for your use case.
